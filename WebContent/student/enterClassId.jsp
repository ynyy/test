...
<title>JP Interactive Class | Enter Class ID</title>
...
<script type="text/javascript">
		 ...
		 function check(){
	    	...
	    	if(gE("classID").value.length!=6){
		        alert("Class ID must be 6-digit number.");
		    	return false;
		    }
		   ...
		   $.ajax({  
  	        ...
  	        cache:true,  
  	        type: 'POST',  
  	        url: "<%=path%>/module?type=judgeClassIDExist&classID="+gE("classID").value, /* 请求的action路径 */  
  	        ...
  	        success:function(data){ //请求成功后处理函数。    
                 if(data.indexOf("success")>=0){
                     window.location.href = "<%=path%>/message?type=stuForumDetail&classID=" + gE("classID").value;
				}else{
					alert(data);
				}
			}
...
<body>
	...
	<!--Enter ClassID-->
	<div class="tit01">
		<strong>Enter Class ID</strong>
	</div>
	<form name="formAdd" method="post" onsubmit="return check()">
		<div class="publick_box cx">
			<p>
				<span class="classID">ClassID：</span>
				<span class="zdtxt"> <input type="text" id="classID" name="classID" /></span>
			</p>
